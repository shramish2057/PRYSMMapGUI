...
to run the coral model for time period 1985-1994 and the passed location coordinates.
Hence, the SST and SSS datasets of 1985-1994 collected from World Ocean Atlas are used
as input datasets. 
"""
...
def coral_driver(latitude, longitude):
    ...
    with open(sstdatadir+'woa18_8594_t00mn01.csv', 'r') as csv_file:
        ...
        with open(sstdatadir+'woa18_8594_sst.csv', 'w') as new_file:
    ...
    #reading the World Ocean Atlas Sea Surface Salinity csv dataset for 1985-1994
    with open(sssdatadir+'woa18_8594_s00mn01.csv', 'r') as csv_file:
        csv_reader = csv.reader(csv_file)
        ...
        # This hence saves the sea surface temperature dataset for specific location
        # for depths ranging from 0-5500 m.
        with open(sssdatadir+'woa18_8594_sss.csv', 'w') as new_file:
            csv_writer = csv.writer(new_file, delimiter='\n')
    ...
    # yearly
    file = open(sstdatadir+'woa18_8594_sst.csv')
    ssta = np.loadtxt(file, delimiter=",")

    file = open(sssdatadir +'woa18_8594_sss.csv')
